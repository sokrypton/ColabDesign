...
- **28Feb2022** - updating definition of [con]tact loss from `-log(softmax(logits)[:-1].sum())` to `-log(softmax(logits)[:-1].max())`. The original definition tried to maximize number of contacts within a cutoff, while the new definition also minimizes the entropy within the cutoff.
# FAQ

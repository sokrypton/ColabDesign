...
def setup_crops(self, crop_len=32, crop_mode="slide"):
  ...
  # callback functions
  def pre_crop(inputs, opt):
    p = opt["crop_pos"]
    inputs = crop_feat(inputs, p)    
    inputs["prev"]["prev_msa_first_row"] = inputs["prev"]["prev_msa_first_row"][p]
    inputs["prev"]["prev_pair"]          = inputs["prev"]["prev_pair"][p[:,None],p[None,:]]
    inputs["prev"]["prev_pos"]           = inputs["prev"]["prev_pos"][p]
  ...
  if crop:
    self.pre_callback.append(pre_crop)
    self.post_callback.append(post_crop)
    self.design_callback.append(design_crop)
